...
import com.quberratrix.identity.repository.EmailVerificationTokenRepository;
import com.quberratrix.identity.repository.PasswordResetTokenRepository;
...
public class TokenServices {
    ...
    @Transactional
    public Mono<Void> requestEmailVerification(String email, String ipAddress, String userAgent) {
                ...
                .flatMap(user -> {
                    ...
                    return emailVerificationTokenRepository.save(token)
                            .flatMap(saved -> auditService.logAndPublishEvent(
                                    ...
                                    user.getId(), user.getId(), null, null, ipAddress, userAgent,
                                    Map.of("email", email, "token", rawToken) // Emitting raw token for Notification Service
                            ));
                }).then();
    }

    @Transactional
    public Mono<Void> verifyEmail(String rawToken, String ipAddress, String userAgent) {
        String hashedToken = sha256(rawToken);
                ...
                .flatMap(token -> {
                    ...
                    return emailVerificationTokenRepository.save(token)
                            ...
                            .flatMap(user -> auditService.logAndPublishEvent(
                                    "USER_VERIFIED",
                                    user.getId(), user.getId(), null, null, ipAddress, userAgent, Map.of()
                            ));
                }).then();
    }

    @Transactional
    public Mono<Void> requestPasswordReset(String email, String ipAddress, String userAgent) {
        return userRepository.findByEmail(email)
                .flatMap(user -> {
                    String rawToken = generateOpaqueToken();
                    String hashedToken = sha256(rawToken);
    ...
    @Transactional
    public Mono<Void> confirmPasswordReset(String rawToken, String newPassword, String ipAddress, String userAgent) {
        String hashedToken = sha256(rawToken);
